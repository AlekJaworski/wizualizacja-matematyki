---
options:
  - "$x = \frac{\pi}{6} + 2k\pi$ lub $x = \frac{5\pi}{6} + 2k\pi$, $k \in \mathbb{Z}$"
  - "$x = \frac{\pi}{6} + k\pi$, $k \in \mathbb{Z}$"
  - "$x = \frac{\pi}{3} + 2k\pi$, $k \in \mathbb{Z}$"
  - "$x = \frac{\pi}{6} + k\pi$ lub $x = -\frac{\pi}{6} + k\pi$"
optionsEn:
  - "$x = \frac{\pi}{6} + 2k\pi$ or $x = \frac{5\pi}{6} + 2k\pi$, $k \in \mathbb{Z}$"
  - "$x = \frac{\pi}{6} + k\pi$, $k \in \mathbb{Z}$"
  - "$x = \frac{\pi}{3} + 2k\pi$, $k \in \mathbb{Z}$"
  - "$x = \frac{\pi}{6} + k\pi$ or $x = -\frac{\pi}{6} + k\pi$"
correct: 0
hint: "$\sin x = \frac{1}{2}$ dla $x = \frac{\pi}{6}$ (30°) i $x = \pi - \frac{\pi}{6} = \frac{5\pi}{6}$ (150°), plus okresy $2\pi$."
hintEn: "$\sin x = \frac{1}{2}$ for $x = \frac{\pi}{6}$ (30°) and $x = \pi - \frac{\pi}{6} = \frac{5\pi}{6}$ (150°), plus periods of $2\pi$."
tests:
  trig_eq: 1.0
  trig_formulas: 0.5
---
Rozwiąż równanie: $\sin x = \dfrac{1}{2}$

<!-- en -->
Solve the equation: $\sin x = \dfrac{1}{2}$