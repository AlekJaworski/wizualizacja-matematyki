---
source: cke_2024_rozszerzona_zad1
options:
  - "$59$ °C"
  - "$50$ °C"
  - "$53$ °C"
  - "$56$ °C"
optionsEn:
  - "$59$ °C"
  - "$50$ °C"
  - "$53$ °C"
  - "$56$ °C"
correct: 0
hints:
  - "① Model chłodzenia Newtona: $T(t)=(T_p-T_z)k^{-t}+T_z$. Z danych: $T_p=80$, $T_z=20$, $T(10)=65$ — wyznacz $k$."
  - "② $65=60\cdot k^{-10}+20\Rightarrow k^{-10}=\tfrac{45}{60}=\tfrac{3}{4}$. Szukane: $T(15)=60\cdot k^{-15}+20=60\cdot k^{-10}\cdot k^{-5}+20$."
  - "③ $k^{-5}=\sqrt{k^{-10}}=\sqrt{0{,}75}\approx 0{,}866$. Stąd $T(15)=60\cdot 0{,}75\cdot 0{,}866+20\approx 38{,}97+20\approx 59$. Po zaokrągleniu do jedności — $59$ °C. Uwaga: w zadaniu CKE pytano o temperaturę za $5$ minut od chwili $t=10$, tu ten sam rachunek."
hintsEn:
  - "① Newton's cooling model: $T(t)=(T_p-T_z)k^{-t}+T_z$. From the data: $T_p=80$, $T_z=20$, $T(10)=65$ — find $k$."
  - "② $65=60\cdot k^{-10}+20\Rightarrow k^{-10}=\tfrac{45}{60}=\tfrac{3}{4}$. Wanted: $T(15)=60\cdot k^{-15}+20=60\cdot k^{-10}\cdot k^{-5}+20$."
  - "③ $k^{-5}=\sqrt{k^{-10}}=\sqrt{0.75}\approx 0.866$. Hence $T(15)=60\cdot 0.75\cdot 0.866+20\approx 38.97+20\approx 59$. Rounded to integers — $59$ °C. Note: the CKE problem asked for the temperature $5$ minutes after $t=10$ — same calculation."
tests:
  log_exp: 1.0
  fn_exp: 0.5
---
Temperatura stygnącej kawy zmienia się wg wzoru $T(t)=(T_p-T_z)k^{-t}+T_z$, gdzie $T_p=80$ °C, $T_z=20$ °C. Po $10$ minutach kawa ma $65$ °C. Jaka będzie jej temperatura po kolejnych $5$ minutach (zaokrąglij do jedności)?

<!-- en -->
The temperature of cooling coffee changes according to the formula $T(t)=(T_p-T_z)k^{-t}+T_z$, where $T_p=80$ °C, $T_z=20$ °C. After $10$ minutes the coffee is at $65$ °C. What will its temperature be after another $5$ minutes (round to integers)?
