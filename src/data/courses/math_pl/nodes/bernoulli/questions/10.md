---
source: cke_2024_rozszerzona_zad3
options:
  - "$\approx 0{,}996$"
  - "$\approx 0{,}990$"
  - "$\approx 0{,}904$"
  - "$\approx 0{,}099$"
optionsEn:
  - "$\approx 0.996$"
  - "$\approx 0.990$"
  - "$\approx 0.904$"
  - "$\approx 0.099$"
correct: 0
hints:
  - "① $n=10$ niezależnych prób, prawdopodobieństwo \"sukcesu\" (wadliwa śmietana) $p=0{,}01$. Szukamy $P(k\le 1)=P(k=0)+P(k=1)$."
  - "② $P(k=0)=(0{,}99)^{10}$, $P(k=1)=\binom{10}{1}(0{,}01)(0{,}99)^9=10\cdot 0{,}01\cdot (0{,}99)^9$."
  - "③ $(0{,}99)^{10}\approx 0{,}9044$, $10\cdot 0{,}01\cdot (0{,}99)^9\approx 0{,}0914$. Suma $\approx 0{,}996$."
hintsEn:
  - "① $n=10$ independent trials, probability of \"success\" (defective cream) $p=0.01$. We want $P(k\le 1)=P(k=0)+P(k=1)$."
  - "② $P(k=0)=(0.99)^{10}$, $P(k=1)=\binom{10}{1}(0.01)(0.99)^9=10\cdot 0.01\cdot (0.99)^9$."
  - "③ $(0.99)^{10}\approx 0.9044$, $10\cdot 0.01\cdot (0.99)^9\approx 0.0914$. Sum $\approx 0.996$."
tests:
  bernoulli: 1.0
  probability: 0.3
---
Prawdopodobieństwo, że w losowym opakowaniu śmietany zawartość tłuszczu jest mniejsza niż $36\%$, wynosi $0{,}01$. Badamy $10$ opakowań. Oblicz prawdopodobieństwo, że co najwyżej jedno z nich będzie miało zbyt mało tłuszczu (zaokrąglij do części tysięcznych).

<!-- en -->
The probability that a random package of cream has fat content less than $36\%$ is $0.01$. We examine $10$ packages. Calculate the probability that at most one of them has too little fat (round to the nearest thousandth).
