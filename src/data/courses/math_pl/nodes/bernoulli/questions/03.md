---
options:
  - "$\frac{3}{8}$"
  - "$\frac{1}{8}$"
  - "$\frac{1}{2}$"
  - "$\frac{1}{4}$"
optionsEn:
  - "$\frac{3}{8}$"
  - "$\frac{1}{8}$"
  - "$\frac{1}{2}$"
  - "$\frac{1}{4}$"
correct: 0
hint: "$P(X=2) = \binom{3}{2} \cdot (\frac{1}{2})^2 \cdot \frac{1}{2} = 3 \cdot \frac{1}{4} \cdot \frac{1}{2} = \frac{3}{8}$."
hintEn: "$P(X=2) = \binom{3}{2} \cdot (\frac{1}{2})^2 \cdot \frac{1}{2} = 3 \cdot \frac{1}{4} \cdot \frac{1}{2} = \frac{3}{8}$."
tests:
  bernoulli: 1.0
  probability: 0.5
---
Rzucamy monetą 3 razy ($p = \frac{1}{2}$). Prawdopodobieństwo dokładnie 2 orłów wynosi:

<!-- en -->
We toss a coin 3 times ($p = \frac{1}{2}$). The probability of exactly 2 heads is: