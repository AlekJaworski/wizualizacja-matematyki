---
source: cke_2024_rozszerzona_zad6
options:
  - "$11\,040$"
  - "$12\,000$"
  - "$7\,200$"
  - "$10\,080$"
optionsEn:
  - "$11\,040$"
  - "$12\,000$"
  - "$7\,200$"
  - "$10\,080$"
correct: 0
hints:
  - "① Dokładnie $3$ nieparzyste + $2$ parzyste + wszystkie różne oznacza liczbę $5$-cyfrową. Nieparzyste $\{1,3,5,7,9\}$ — wybór $\binom{5}{3}=10$. Parzyste $\{0,2,4,6,8\}$ — wybór $\binom{5}{2}=10$. Razem $100$ par zbiorów cyfr."
  - "② Rozważ dwa przypadki w zależności od tego, czy $0$ został wybrany. Gdy $0$ NIE jest wybrany ($\binom{4}{2}=6$ sposobów dla parzystych), wszystkie $5!=120$ ustawień jest poprawne. Gdy $0$ jest wybrany ($\binom{4}{1}=4$ sposobów), odrzucamy ustawienia zaczynające się od $0$: $5!-4!=96$."
  - "③ Bez $0$: $\binom{5}{3}\cdot\binom{4}{2}\cdot 5!=10\cdot 6\cdot 120=7200$. Z $0$: $\binom{5}{3}\cdot\binom{4}{1}\cdot 96=10\cdot 4\cdot 96=3840$. Suma: $7200+3840=11\,040$."
hintsEn:
  - "① Exactly $3$ odd + $2$ even + all distinct means a $5$-digit number. Odd digits $\{1,3,5,7,9\}$ — choose $\binom{5}{3}=10$. Even digits $\{0,2,4,6,8\}$ — choose $\binom{5}{2}=10$. In total $100$ pairs of digit sets."
  - "② Consider two cases depending on whether $0$ was chosen. When $0$ is NOT chosen ($\binom{4}{2}=6$ ways for the even digits), all $5!=120$ arrangements are valid. When $0$ IS chosen ($\binom{4}{1}=4$ ways), we exclude arrangements starting with $0$: $5!-4!=96$."
  - "③ Without $0$: $\binom{5}{3}\cdot\binom{4}{2}\cdot 5!=10\cdot 6\cdot 120=7200$. With $0$: $\binom{5}{3}\cdot\binom{4}{1}\cdot 96=10\cdot 4\cdot 96=3840$. Total: $7200+3840=11\,040$."
tests:
  counting: 1.0
  permutations: 0.5
---
Ile jest liczb naturalnych, w których zapisie dziesiętnym żadna cyfra się nie powtarza, dokładnie trzy cyfry są nieparzyste i dokładnie dwie cyfry są parzyste?

<!-- en -->
How many natural numbers are there whose decimal representation has no repeated digit, with exactly three odd digits and exactly two even digits?
